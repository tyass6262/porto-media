...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .profile-email { font-size: 12.5px; color: var(--text-muted); margin-top: 2px; }
        ...
        .profile-date i { font-size: 10px; opacity: 0.5; }
        ...
        .form-card {
            ...
            border-radius: 16px;
            padding: 32px;
            animation: fu 0.45s cubic-bezier(0.16,1,0.3,1) both;
...
<header class="topbar">
    <div class="topbar-left">
        ...
        <div class="breadcrumb-bar">
            ...
            <span class="sep"><i class="fas fa-chevron-right"></i></span>
            <span class="current">{{ Str::limit($user->name, 20) }}</span>
        </div>
    </div>
    <div class="topbar-right">
        <div class="topbar-chip"><i class="fas fa-pen-to-square"></i> Edit</div>
        <div class="user-pill">
...
<main class="page">
    ...
    <div class="profile-card">
        @php
            ...
            $ci = ord(substr($user->name, 0, 1)) % count($avColors);
        @endphp
        <div class="profile-av" style="{{ $avColors[$ci] }}">{{ substr($user->name, 0, 2) }}</div>
        <div class="profile-info">
            <div class="profile-name">{{ $user->name }}</div>
            <div class="profile-email">{{ $user->email }}</div>
            <div class="profile-meta">
                <span class="role-pill {{ $rc }}"><span class="dot"></span>{{ $rl }}</span>
                <span class="profile-date"><i class="fas fa-calendar"></i> {{ $user->created_at->format('d M Y') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="form-card">
        <div class="form-card-head">
            <i class="ic-green"><i class="fas fa-user-pen"></i></i>
        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" id="userForm">
            ...
            <div class="opt-div"><span>Ubah Password</span></div>

            <div class="fg">
                <label for="password">Password Baru <span class="opt">(kosongkan jika tidak diubah)</span></label>
                <div class="iw">
                    <i class="fas fa-key iw-icon"></i>
                    <input type="password" id="password" name="password"
                        class="fi {{ $errors->has('password') ? 'err' : '' }}"
                        placeholder="Minimal 8 karakter" minlength="8" autocomplete="new-password">
                    <button type="button" class="toggle-pw" onclick="togglePw('password','eye1')">
                        <i class="fas fa-eye" id="eye1"></i>
                    </button>
                </div>
                <div class="str-wrap" id="strWrap" style="opacity:0;transition:opacity 0.25s;">
            ...
            <div class="fg" id="confGroup" style="opacity:0;transition:opacity 0.25s;">
                <label for="password_confirmation">Konfirmasi Password Baru</label>
                <div class="iw">
                    <i class="fas fa-lock iw-icon"></i>
                    <input type="password" id="password_confirmation" name="password_confirmation"
                        class="fi" placeholder="Ulangi password baru" autocomplete="new-password">
            ...
            <div class="field-hint" style="margin-top:4px;">
                ...
                <span>Biarkan kolom password kosong jika tidak ingin mengubahnya.</span>
            </div>
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn-submit" id="btnSub">
                    ...
                    <span id="btnTxt">Simpan Perubahan</span>
                </button>
            </div>
        </form>
    </div>
...
<script>
    ...
    var strWrap = document.getElementById('strWrap');
    var confGroup = document.getElementById('confGroup');
    var segs = [document.getElementById('s1'),document.getElementById('s2'),document.getElementById('s3'),document.getElementById('s4')];
    ...
    pwIn.addEventListener('input', function() {
        var v = this.value;
        var has = v.length > 0;
        strWrap.style.opacity = has ? '1' : '0';
        confGroup.style.opacity = has ? '1' : '0';
        if (has) {
            var sc = getScore(v);
            ...
            segs.forEach(function(s) { s.style.background = 'rgba(255,255,255,0.06)'; });
